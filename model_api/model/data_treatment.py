...
def augment_and_save_images(
    image_path, save_dir, num_augmented=2, img_width=150, img_height=150
):
    ...
    for i, batch in enumerate(
        datagen.flow(
            ...
            save_prefix=os.path.splitext(base_name)[0] + "_aug",
            save_format="jpg",
        )
    ):
        if i >= num_augmented:
...
def augment_dataset(
    dataset_path, save_base_dir, split, img_width=150, img_height=150, num_augmented=2
):
        ...
        for file in os.listdir(category_path):
            if file.endswith(".jpg"):
                ...
                augment_and_save_images(
                    file_path, save_dir, num_augmented, img_width, img_height
                )
